...
=head1 SYNOPSIS
    ...
    # or
    $app->plugin(RoutesAuthDBI); # will try apply dbh=> $app->dbh

The config attr B<dbh> - DBI connection where are tables: routes, users, roles, rels.
...
All tables has primary column id from one sequence. 

=over 4

=item * Pg sequence for all tables
...
=item * Table of relations between routes, users and roles

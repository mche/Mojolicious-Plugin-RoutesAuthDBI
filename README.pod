...
=item * B<access> - hashref options for special access module. This module has subs and methods for manage auth and access operations, has appling routes from sql-table or other sources. By default plugin will load the builtin module:
...
=item * B<admin> - hashref options for admin controller for actions on SQL tables: controllers, actions, routes, roles, users. By default the builtin module:
...
If you changed the routes table then kill -HUP or reload app to regenerate routes. Changing other tables will not require reloading the service.

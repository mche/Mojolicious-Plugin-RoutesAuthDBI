...
Mojolicious::Plugin::RoutesAuthDBI - Generate routes from sql-table and make restrict access to them with users/roles tables. Plugin makes an auth operations throught the plugin L<Mojolicious::Plugin::Authentication> on which is based.

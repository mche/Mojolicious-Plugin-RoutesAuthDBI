...
sub startup {# 
  ...
  my $r = $app->routes;
  $r->route('/callback')->over(access=>{auth=>1, role=>'admin00'})->to(cb => sub {shift->render(format=>'txt', text=>'You have access!')})->name('foo');#'install#manual', namespace000=>'Mojolicious::Plugin::RoutesAuthDBI',
  ...
  $r->route('/тест1')->over(access=>{auth=>1,})->to('test#test1', namespace0=>'вася',);

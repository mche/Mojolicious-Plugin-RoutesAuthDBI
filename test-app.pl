...
sub startup {# 
  ...
  my $r = $app->routes;
  $r->route('/fooadmin')->over(access=>{auth=>1, role=>'admin0'})->to(cb => sub {shift->render(format=>'txt', text=>'You have assess!')})->name('foo');#'install#manual', namespace000=>'Mojolicious::Plugin::RoutesAuthDBI',
}
